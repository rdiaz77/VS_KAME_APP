# === clean_collection.py (final version with formatted numbers) ===
import pandas as pd
import os

def clean_collection(
    input_path="test/cuentas_por_cobrar_full.csv",
    output_path="test/cuentas_por_cobrar_clean.csv"
):
    """
    Clean the 'Cuentas por Cobrar' (collection) dataset.

    Steps:
    - Drops unnecessary columns: Cuenta, MultiDirNombre, Direccion
    ...
    drop_cols = ["Cuenta", "MultiDirNombre", "Direccion"]
...
if __name__ == "__main__":
    df_clean = clean_collection()
    print(f"✅ Done. Cleaned {len(df_clean)} rows.")
# === END clean_collection.py ===
